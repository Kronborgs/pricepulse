    ...
    async def send_due_digests(self) -> None:
        ...
        async with AsyncSessionLocal() as db:
            ...
            for rule in rules:
                ...
                # Find alle watches for denne bruger (v1)
                watches_result = await db.execute(
                    select(Watch).where(
                        Watch.owner_id == rule.user_id,
                ...
                for ev, watch_obj, product in rows:
                    ...
                    shop_name = watch_obj.url.split("/")[2] if watch_obj.url else "ukendt"
                ...
                # ── Aktuelle priser fra v1 watches (aktivt datasystem) ──────────
